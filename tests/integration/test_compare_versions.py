...
def _create_prompt_with_versions(storage: Storage, prompt_name: str, contents: list[str]):
    ...
    for seq, content in enumerate(contents, start=1):
        prompt.add_version(content=content, name=f"v{seq}", author="qa", message=f"version {seq}")
    return prompt
...
def test_uc07_003_compare_versions_line_diff_identical_has_no_changes(tmp_path):
    ...
    try:
        ...
        prompt.add_version(content="stable content", name="v1")
...
def test_uc07_006_compare_versions_raises_for_unknown_version(tmp_path):
    ...
    try:
        ...
        prompt.add_version(content="some content", name="v1")
...
def test_uc07_007_compare_versions_chars_chunks_cover_full_content(tmp_path):
    ...
    try:
        ...
        content_b = "Hi Python world!"
        prompt = _create_prompt_with_versions(storage, "chunks_cover_prompt", [content_a, content_b])

        diff = prompt.compare_versions_chars("v1", "v2")

        reconstructed_a = "".join(c.old_text for c in diff.chunks)
        reconstructed_b = "".join(c.new_text for c in diff.chunks)
        assert reconstructed_a == content_a
        assert reconstructed_b == content_b
    finally:
        storage._conn.close()
